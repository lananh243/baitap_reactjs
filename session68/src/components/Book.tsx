...
export default function Book() {
    ...
    const validateData = (): boolean => {
        ...
        if (!titleBook) {
            errors.titleBook = "Tên sách không được để trống.";
            isValid = false;
        }

        if (!nameStudent) {
            errors.nameStudent = "Tên người mượn không được để trống.";
            isValid = false;
        }
        ...
        if (!borrowDay) {
            errors.borrowDay = "Ngày mượn không được để trống.";
            isValid = false;
        } else if (borrowDay < today) {
            errors.borrowDay = "Ngày mượn không được bé hơn ngày hiện tại.";
            isValid = false;
        } else if (borrowDay > payDay) {
            errors.borrowDay = "Ngày mượn không được lớn hơn ngày trả";
            isValid = false;
        }

        if (!payDay) {
            errors.payDay = "Ngày trả không được để trống.";
            isValid = false;
        } else if (payDay < today) {
            errors.payDay = "Ngày trả không được bé hơn ngày hiện tại.";
            isValid = false;
        }
        ...
        <div className='container'>
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Form>
                    ...
                    />
                    {error.titleBook && <div className="error-message" style={{color:"red"}}>{error.titleBook}</div>}
                    <Form.Label>Tên người mượn</Form.Label>
                    <Form.Control
                        type="text"
                        onChange={handleChangeName}
                    />
                    {error.nameStudent && <div className="error-message" style={{color:"red"}}>{error.nameStudent}</div>}
                    ...
                    />
                    {error.borrowDay && <div className="error-message" style={{color:"red"}}>{error.borrowDay}</div>}
                    ...
                    />
                    {error.payDay && <div className="error-message" style={{color:"red"}}>{error.payDay}</div>}
                    </Form.Group>
                </Form>
                </Modal.Body>
                <Modal.Footer>
                <Button style={{width:"100%"}} variant="primary" onClick={addBook}>
                    Thêm
                </Button>
                </Modal.Footer>
            </Modal>
                    ...
                    {books.bookReducer.map((book:BookTodo, index:number) => {
                        return (
                            <tr key={book.id}>
                                ...
                                <td>{book.payDay}</td>
                                <td style={{width:"150px"}} onClick={() => {handleUpdateStatus(book.id)}}>
                                    {book.status ? <button style={{width:"100px", borderRadius:"20px", backgroundColor:"yellowgreen", border:"none"}}>Đã trả</button> : <button style={{width:"100px", borderRadius:"20px", backgroundColor:"red", border:"none"}}>Chưa trả</button>}
                                </td>
